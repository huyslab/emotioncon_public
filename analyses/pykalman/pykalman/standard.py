...
def _em(observations, transition_offsets, observation_offsets,
    ...
    if 'transition_matrices' not in given and 'control_matrix' not in given:
        transition_matrix, control_matrix = _em_transition_control_matrices(
        ...
        if 'transition_matrices' in given:
            ...
            transition_matrix = _em_transition_matrix(
                transition_offsets, smoothed_state_means,
                smoothed_state_covariances, pairwise_covariances,
                given['control_matrix'], control_inputs
            )
        if 'control_matrix' in given:
            ...
            control_matrix = _em_control_matrix(
                transition_offsets, smoothed_state_means,
                smoothed_state_covariances, pairwise_covariances,
                given['transition_matrices'], control_inputs
            )
...
def _em_transition_matrix(transition_offsets, smoothed_state_means,
                                    smoothed_state_covariances, pairwise_covariances,
                                    control_matrix, control_inputs=None):
    """Adapted by Jolanda Malamud to estimate transition matrix if 
    ...
    for t in range(1, n_timesteps):
        ...
        if control_inputs is not None:
            res1 += (- np.dot(control_matrix, np.outer(control_inputs[t], smoothed_state_means[t - 1])))
...
def _em_control_matrix(transition_offsets, smoothed_state_means,
                       smoothed_state_covariances, pairwise_covariances,
                       transition_matrix, control_inputs=None):
    ...
    for t in range(1, n_timesteps):
        transition_offset = _last_dims(transition_offsets, t - 1, ndims=1)
        res1 += (
            np.outer(smoothed_state_means[t], control_inputs[t])
            - np.dot(transition_matrix, np.outer(smoothed_state_means[t - 1], control_inputs[t]))
            - np.outer(transition_offset, control_inputs[t])
        )

        res2 += (np.outer(control_inputs[t], control_inputs[t]))
